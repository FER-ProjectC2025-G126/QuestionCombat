export default class Questions {
  ...
  // Get questions by status (for admin approval workflow)
  getQuestionsByStatus(status = 'PENDING') {
    return new Promise((resolve, reject) => {
      this.db.all(
        'SELECT q.*, u.username as created_by_name FROM questions q LEFT JOIN users u ON q.created_by = u.username WHERE q.status = ? ORDER BY q.created_at DESC',
        [status],
        (err, rows) => {
          if (err) {
            reject(err);
